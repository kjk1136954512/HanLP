...
from hanlp_common.constant import HF_MIRROR
...
from transformers import BertTokenizer, BertConfig, PretrainedConfig, \
    ...
    AutoModelForTokenClassification, optimization, BartModel, AlbertModel


class AutoModel_(AutoModel):
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, training=True, **kwargs) -> PreTrainedModel:
        if training:
            return super().from_pretrained(pretrained_model_name_or_path, *model_args, mirror=mirror, **kwargs)
        else:
